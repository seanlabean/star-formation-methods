...
def main(m, z, end_time, time_step, unfmt):
    stellar1 = Seba()
    stellar2 = Seba()
    stellar1.parameters.metallicity = z
    stellar2.parameters.metallicity = z
    stellar1.particles.add_particle(Particle(mass=m))
    stellar2.particles.add_particle(Particle(mass=m))
    #print_star(stellar1, unfmt)
    
    while stellar1.model_time < end_time-0.5*time_step:
        stellar1.evolve_model(stellar1.model_time+time_step)
        print("evolve model:")
        print_star(stellar1, unfmt)
        _tmp = stellar2.evolve_star(m, stellar1.model_time+time_step, 0.02)
        ...
        print("evolve star:")
        print("t =", stellar1.model_time, " M = {:.2e}".format(se_mass.value_in(units.MSun)), "MSun",
              " R = {:.2e}".format(se_radius.value_in(units.RSun)), "RSun",
              " L = {:.2e}".format(se_lum.value_in(units.LSun)), "LSun ",  se_type)
        print(" ")
        stellar1.model_time += time_step
        if stellar1.particles.stellar_type.number[0] >= 10 and se_type.value_in(units.stellar_type) >= 10: break	# remnant
